              ...
              <table className="w-full border-collapse border border-gray-300">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="border border-gray-300 px-2 py-1 text-left text-xs">거리(km)</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={delivery.id} className="hover:bg-gray-50">
                      ...
                      <td className="border border-gray-300 px-2 py-1 text-xs">
                        <span className={`px-1 py-0.5 rounded text-xs ${
                          ...
                          delivery.status === '배차완료' ? 'bg-yellow-100 text-yellow-800' :
                          delivery.status === '접수완료' ? 'bg-gray-100 text-gray-800' :
                          delivery.status === '수거중' ? 'bg-purple-100 text-purple-800' :
                          delivery.status === '수거완료' ? 'bg-indigo-100 text-indigo-800' :
                          delivery.status === '조처완료' ? 'bg-teal-100 text-teal-800' :
                          delivery.status === '배송취소' ? 'bg-red-100 text-red-800' :
                          delivery.status === '배송연기' ? 'bg-orange-100 text-orange-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {delivery.status}
                        </span>
                      </td>
                      <td className="border border-gray-300 px-2 py-1 text-xs">{delivery.driver_id}</td>
                      <td className="border border-gray-300 px-2 py-1 text-xs">
                        {delivery.installation_photos && delivery.installation_photos.length > 0 ? (
                          <span className="text-blue-600 text-xs">{delivery.installation_photos.length}장</span>
                        ) : (
                      ...
                      <td className="border border-gray-300 px-2 py-1 text-xs">{delivery.distance ? `${delivery.distance}km` : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
