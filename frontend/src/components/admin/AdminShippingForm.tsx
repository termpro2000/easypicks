...
import { deliveriesAPI, requestTypesAPI, userAPI } from '../../services/api';
...
const AdminShippingForm: React.FC<AdminShippingFormProps> = ({ onNavigateBack }) => {
  ...
  const handleCompanySelect = (company: UserOption) => {
    ...
    // 발송인 정보 자동 설정
    if (company.default_sender_name) {
      setValue('sender_name', company.default_sender_name);
    } else if (company.name) {
      setValue('sender_name', company.name);
    }

    if (company.default_sender_address) {
      setValue('sender_address', company.default_sender_address);
    }

    if (company.default_sender_detail_address) {
      setValue('sender_detail_address', company.default_sender_detail_address);
    }

    if (company.default_sender_company || company.company) {
      setValue('furniture_company', company.default_sender_company || company.company);
    }

    if (company.default_sender_phone || company.phone) {
      setValue('emergency_contact', company.default_sender_phone || company.phone);
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  ...
                  {selectedUser && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                        ...
                        {(selectedUser.default_sender_phone || selectedUser.phone) && (
                          <div>
                            <span className="text-blue-700 font-medium">연락처:</span>
                            <span className="ml-2 text-blue-800">
                              {selectedUser.default_sender_phone || selectedUser.phone}
                            </span>
                          </div>
                        )}
                        {selectedUser.default_sender_address && (
                          <div className="md:col-span-2">
                            <span className="text-blue-700 font-medium">주소:</span>
                            <span className="ml-2 text-blue-800">
                              {selectedUser.default_sender_address}
                              {selectedUser.default_sender_detail_address && 
                                `, ${selectedUser.default_sender_detail_address}`
                              }
                            </span>
                          </div>
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="발송인 이름 (sender_name)" icon={User} required error={errors.sender_name?.message}>
                <input
                  type="text"
              ...
              <InfoCell label="발송인 주소 (sender_addr)" icon={MapPin} required error={errors.sender_address?.message}>
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      type="text"
              ...
              <InfoCell label="가구회사 (furniture_company)" icon={Building}>
                <input
                  type="text"
              ...
              <InfoCell label="긴급연락처 (emergency_contact)" icon={Phone}>
                <input
                  type="tel"
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="고객 이름 (customer_name)" icon={User} required error={errors.customer_name?.message}>
                <input
                  type="text"
              ...
              <InfoCell label="고객 전화번호 (customer_phone)" icon={Phone} required error={errors.customer_phone?.message}>
                <input
                  type="tel"
              ...
              <InfoCell label="고객 주소 (customer_addr)" icon={MapPin} required error={errors.customer_address?.message}>
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      type="text"
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="의뢰종류 (request_type)" icon={FileText}>
                <select
              ...
              <InfoCell label="시공 유형 (construction_type)" icon={Wrench}>
                <select
              ...
              <InfoCell label="방문 날짜 (visit_date)" icon={Calendar}>
                <input
                  type="date"
                  {...register('visit_date')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <InfoCell label="방문 시간 (visit_time)" icon={Clock}>
                <input
                  type="time"
                  {...register('visit_time')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <InfoCell label="배정된 기사 (assigned_driver)" icon={Truck}>
                <input
                  type="text"
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="건물 유형 (building_type)" icon={Home} required error={errors.building_type?.message}>
                <select
              ...
              <InfoCell label="층수 (floor_count)" icon={Building} required error={errors.floor_count?.message}>
                <input
                  type="number"
                  min="1"
              ...
              <InfoCell label="접근성 옵션 (elevator_available, ladder_truck)" icon={Settings}>
                <div className="space-y-2">
              ...
              <InfoCell label="작업 유형 (disposal, room_movement, wall_construction)" icon={Wrench}>
                <div className="space-y-2">
                  <label className="flex items-center gap-2">
                    <input
                      type="checkbox"
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="제품명 (product_name)" icon={Package} required error={errors.product_name?.message}>
                <div className="flex gap-2">
                  <input
                    type="text"
              ...
              <InfoCell label="제품 코드 (furniture_product_code)" icon={FileText}>
                <input
                  type="text"
              ...
              <InfoCell label="제품 무게 (kg) (product_weight)" icon={Weight}>
                <input
                  type="number"
                  min="0"
                  step="0.1"
              ...
              <InfoCell label="전체 무게 (kg) (weight)" icon={Weight}>
                <input
                  type="number"
                  min="0"
                  step="0.1"
              ...
              <InfoCell label="제품 크기 (product_size)" icon={Box}>
                <input
                  type="text"
              ...
              <InfoCell label="박스 크기 (box_size)" icon={Box}>
                <input
                  type="text"
                  {...register('box_size')}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <InfoCell label="배송비 (원) (delivery_fee)" icon={Shield}>
                <input
                  type="number"
                  min="0"
              ...
              <InfoCell label="보험가치 (원) (insurance_value)" icon={Shield}>
                <input
                  type="number"
                  min="0"
              ...
              <InfoCell label="착불금액 (원) (cod_amount)" icon={Shield}>
                <input
                  type="number"
                  min="0"
                  {...register('cod_amount')}
              ...
              <InfoCell label="배송 시간 선호도 (delivery_time_preference)" icon={Clock}>
                <select
              ...
              <InfoCell label="예상 배송일 (estimated_delivery)" icon={Calendar}>
                <input
                  type="datetime-local"
              ...
              <InfoCell label="특별 옵션 (fragile)" icon={AlertTriangle}>
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    {...register('fragile')}
          ...
          <div className="bg-white rounded-xl shadow-lg border p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <InfoCell label="가구사 요청사항 (furniture_requests)" icon={FileText}>
                <textarea
              ...
              <InfoCell label="메인 메모 (main_memo)" icon={FileText}>
                <textarea
                  {...register('main_memo')}
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <InfoCell label="기사님 메모 (driver_notes)" icon={Truck}>
                <textarea
              ...
              <InfoCell label="특별 지시사항 (special_instructions)" icon={AlertTriangle}>
                <textarea
              ...
              <InfoCell label="상세 메모 (detail_notes)" icon={FileText}>
                <textarea
