...
const sampleDeliveries = [
  ...
  },
  {
    ...
    customer_address: '울산시 남구 삼산로 987',
    package_type: '특송',
    weight: 1.2,
    status: 'pending',
    request_type: '새가구 배송',
...
async function addSampleDeliveries() {
  try {
      ...
      try {
        await pool.execute(`
          INSERT INTO deliveries (
            ...
            package_type, weight, status, request_type, construction_type, shipment_type,
            visit_date, visit_time, assigned_driver, furniture_company, main_memo,
            emergency_contact, building_type, floor_count, elevator_available,
            ...
            furniture_requests, driver_notes, created_at, updated_at
          ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
        `, [
          delivery.tracking_number, delivery.sender_name, delivery.sender_address,
    ...
    // pending 상태 데이터만 확인
    const [pending] = await pool.execute(`SELECT tracking_number, customer_name, status FROM deliveries WHERE status = 'pending'`);
    console.log(`\n대기중인 배송 (${pending.length}개):`);
    pending.forEach((item, index) => {
      console.log(`${index + 1}. ${item.tracking_number} - ${item.customer_name}`);
    });
